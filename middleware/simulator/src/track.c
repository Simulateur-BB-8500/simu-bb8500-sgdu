...
TRACK_status_t TRACK_init(void) {
    ...
    for (idx = 0; idx < TRACK_STOP_SPEED_HISTORY_SIZE; idx++)
        track_ctx.speed_history_kmh[idx] = 0;
    ...
    // Specific values for last sound.
    track_ctx.speed_range[TRACK_SPEED_RANGE_NUMBER - 1].start_threshold_kmh = 60;
    track_ctx.speed_range[TRACK_SPEED_RANGE_NUMBER - 1].stop_threshold_kmh = 255;
    // Init stop sound.
