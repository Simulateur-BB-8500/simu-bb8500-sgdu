...
void LIGHTS_task(void) {
	...
	case LIGHTS_STATE_OFF:
		if (lights_ctx.status != 0) {
			...
			KEYBOARD_single_press(&ORTS_SHORTCUT_LIGHTS_ON, ORTS_SHORTCUT_PRESS_DURATION_MS_DEFAULT);
			KEYBOARD_single_press(&ORTS_SHORTCUT_LIGHTS_ON, ORTS_SHORTCUT_PRESS_DURATION_MS_DEFAULT);
	...
	case LIGHTS_STATE_ON:
		if (lights_ctx.status == 0) {
			// Send keyboard control.
			KEYBOARD_single_press(&ORTS_SHORTCUT_LIGHTS_OFF, ORTS_SHORTCUT_PRESS_DURATION_MS_DEFAULT);
			KEYBOARD_single_press(&ORTS_SHORTCUT_LIGHTS_OFF, ORTS_SHORTCUT_PRESS_DURATION_MS_DEFAULT);
			lights_ctx.state = LIGHTS_STATE_OFF;
		}
		break;
